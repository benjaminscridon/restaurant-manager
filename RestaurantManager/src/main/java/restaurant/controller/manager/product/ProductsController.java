...
public class ProductsController implements Initializable {
	...
	private void back() {
		try {
			...
			StartManagerApp.getRoot().setCenter(welcome);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	private void toAddProduct() {
		try {
			AnchorPane addProduct = FXMLLoader.load(getClass().getResource("/restaurant/view/manager/product/Add.fxml"));
			StartManagerApp.getRoot().setCenter(addProduct);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
